...
def encrypt(key, fin):
    ...
    while (bv.more_to_read):
        ...
        if len(bitvec) > 0:
            ...
            out_xor = newRE.__xor__( round_keys[0] )
            sub_out = substitute(out_xor)
            perm_out = sub_out.permute(P_Box)
            out_xor = perm_out.__xor__(LE)
            round_out.append(RE.__add__(out_xor))
    for i in range(15):
        for j in range(len(round_out)):
            bitvec = round_out[j]
            [LE, RE] = bitvec.divide_into_two()
            newRE = RE.permute( expansion_permutation )
            out_xor = newRE.__xor__( round_keys[i + 1] )
            sub_out = substitute(out_xor)
            perm_out = sub_out.permute(P_Box)
            out_xor = perm_out.__xor__(LE)
            round_out[j]= RE.__add__(out_xor)
    for i in range(len(round_out)):
        [L, R] = round_out[i].divide_into_two()
        round_out[i] = R.__add__(L)
        round_out[i] = round_out[i].get_hex_string_from_bitvector()
    return(round_out)
...
def decrypt(key, fin):
    ...
    i = 0
    while i < len(hs):
        bitvec = BitVector(hexstring = hs[i :i + 16])
        if len(bitvec) < 64:
                bitvec.pad_from_right(64 - len(bitvec))
        #print(bitvec.get_hex_string_from_bitvector())
        round_out.append(bitvec)
        i += 16
    for j in range(16):
        i = 0
        while i < len(round_out): 
            bitvec = round_out[i]
            [LE, RE] = bitvec.divide_into_two()
            newRE = RE.permute( expansion_permutation )
            out_xor = newRE.__xor__( round_keys[15-j] )
            sub_out = substitute(out_xor)
            perm_out = sub_out.permute(P_Box)
            out_xor = perm_out.__xor__(LE)
            round_out[i] = RE.__add__(out_xor)
            i += 1
    for i in range(len(round_out)):
        #print(round_out[i].get_hex_string_from_bitvector())

        [L, R] = round_out[i].divide_into_two()
        round_out[i] = R.__add__(L)
        round_out[i] = round_out[i].get_text_from_bitvector()
    return(round_out)
