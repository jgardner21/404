...
def img_encrypt(key, fin, fout):
    ...
    for i in range(16):
        for j in range(len(round_out)):
            ...
            out_xor = newRE.__xor__( round_keys[i] )
    ...
    for i in range(len(round_out)):
        round_out[i].write_to_file(FO)

if __name__ == "__main__":
    keyfile = sys.argv[2]
    key = get_encryption_key(keyfile)
    img_encrypt(key, sys.argv[1], sys.argv[3])
